...
| **Vision** | qwen2.5-vl:7b | 7.6B | 5 GB | VRAM (on demand) | Screenshot/image analysis |
...
| qwen2.5-vl:7b | ~5 GB | 4096 | On demand |
...
{ model: 'qwen3-coder-32k', messages, options: { num_ctx: 32768 } }

// Vision — moderate context for image description
{ model: 'qwen2.5-vl:7b', messages, options: { num_ctx: 4096 } }
```
...
User sends message (possibly with screenshot)
  ...
  └──────┬───────┘
         │
         ├── vision ───► qwen2.5-vl:7b (num_ctx: 4096) ──► describe image
         │                      │
         │                      ▼
...
3. **qwen2.5-vl:7b loads on demand** — only when screenshots are present
...
2. Send screenshot + user text to qwen2.5-vl:7b (num_ctx: 4096)
...
3. **`app.js` sendChat** — Add router step before main model call:
   ...
   - If vision: call qwen2.5-vl:7b, then forward to coder
